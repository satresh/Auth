...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        if self.request.user.role=='admin':
            return Product.objects.using('default').all()
        # Filter products to show only those belonging to the user's company
        return Product.objects.using(self.request.user.company).all()

    def perform_create(self, serializer):
        product = serializer.save()
        product._company = self.request.user.company  # Set the company attribute
        product.save(using=self.request.user.company)  # Save to the corresponding database

    def perform_update(self, serializer):
        product = serializer.save()
        product._company = self.request.user.company
        product.save(using=self.request.user.company)